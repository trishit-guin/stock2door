...
export default function AnalyticsPage() {
  ...
  const fetchAnalyticsData = async (showRefreshing = false) => {
    try {
      ...
      const [dashboardResponse, timeSeriesResponse, routeEfficiencyResponse] = await Promise.all([
        apiClient.getDashboardKPIs(),
        apiClient.getTimeSeriesData(timeRange, selectedMetric),
        apiClient.getRouteEfficiencyAnalysis()
      ])
      ...
      if (routeEfficiencyResponse.success) {
        setRouteEfficiencyData(routeEfficiencyResponse.data)
      }
  ...
  const handleTimeRangeChange = async (newTimeRange: string) => {
    ...
    try {
      const timeSeriesResponse = await apiClient.getTimeSeriesData(newTimeRange, selectedMetric)
      if (timeSeriesResponse.success) {
        setTimeSeriesData(timeSeriesResponse.data)
        addNotification({
          type: 'success',
      ...
      addNotification({
        type: 'error',
        message: 'Failed to update time range data'
      })
    }
  }
  ...
  // Refetch time-series data when metric changes
  useEffect(() => {
    if (analyticsData) {
      apiClient.getTimeSeriesData(timeRange, selectedMetric)
        .then(response => {
        ...
        .catch(error => console.error('Metric change error:', error))
    }
