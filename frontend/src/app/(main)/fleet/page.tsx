...
export default function FleetPage() {
  ...
  const fetchVehicles = async () => {
    try {
      ...
      if (response.ok) {
        ...
        addNotification({
          type: 'success',
          message: 'Fleet data loaded successfully'
        })
      } else {
        addNotification({
          type: 'error',
  ...
  const handleAddVehicle = async () => {
    try {
      if (!formData.model || !formData.vehicleNumber) {
        addNotification({
          type: 'error',
  ...
  const handleEditVehicle = async () => {
    try {
      if (!formData.model || !formData.vehicleNumber || !selectedVehicle) {
        addNotification({
          type: 'error',
          message: 'Please fill in all required fields'
        })
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-6">
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border text-center">
          <Battery100Icon className="h-8 w-8 text-green-600 mx-auto mb-3" />
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border text-center">
          <div className="h-8 w-8 bg-yellow-100 rounded-full flex items-center justify-center mx-auto mb-3">
            <span className="text-yellow-700 text-xl font-bold">D</span>
          </div>
          <p className="text-2xl font-bold text-gray-900">
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border text-center">
          <div className="h-8 w-8 bg-purple-100 rounded-full flex items-center justify-center mx-auto mb-3">
            <span className="text-purple-700 text-xl font-bold">C</span>
          </div>
          <p className="text-2xl font-bold text-gray-900">
            {vehicles.filter(v => v.fuelType === 'CNG').length}
          </p>
          <p className="text-sm text-gray-600">CNG</p>
        </div>
        
        <div className="bg-white p-6 rounded-lg shadow-sm border text-center">
          <div className="h-8 w-8 bg-blue-100 rounded-full flex items-center justify-center mx-auto mb-3">
                  ...
                  <div className="mt-4 pt-4 border-t border-gray-100">
                    <div className="grid grid-cols-3 gap-4 text-sm">
                      ...
                      <div>
                        ...
                        <p className="font-medium text-gray-900 capitalize">{vehicle.status.replace('_', ' ')}</p>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="text-gray-900 capitalize">{selectedVehicle.status.replace('_', ' ')}</p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-600 mb-1">
                    Vehicle Type
                ...
                {selectedVehicle.currentLocation && (
                  <div>
                    ...
                    <p className="text-gray-900">{selectedVehicle.currentLocation}</p>
                  </div>
                )}
              </div>
